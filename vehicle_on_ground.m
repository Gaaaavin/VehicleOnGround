...
body_len = 1.5; % half of the length of the vehicle body (m)
body_wid = 1; % half of the width of the vehicle body (m)
axle_len = 0.25; % half of the length of the axle (m)
rin_radius = 1; % raidus of the wheel (m)

g = 9.8;  % gravitational acceleration (m/s^2)
wheel_mass = 0.1;  % mass of wheel (kg)
body_mass = 1; % mass of vechile body (kg)
ground_elastictiy = 2000;  % coefficient of groud elastictiy (s^-2)
ground_damping = 0;  % coefficient of ground damping(s^-1)
rim_elasticity = 20; % coefficient of rim elastictiy (s^-2)
rim_damping = 0; % coefficient of rim damping(s^-1)
position_cm_wheels = [body_len, body_len, -body_len, -body_len;
       body_wid, -body_wid, body_wid, -body_wid; 
       1, 1, 1, 1];  % initial position of centers of mass of wheels
position_cm_body = [0; 0; 1]; % initial position of center of mass of body
...
position_tilde_body = body(body_len, body_wid, axle_len);
...
%% Simulate
wheels_positions = zeros(3, 20, 4, timestep / 5);
body_positions = zeros(3, 8, timestep / 5);
body_velocities = zeros(3, timestep / save_every);
...
% for clock = 1
    ...
    % ground
    for i = 1:4
        for j = 1:num_rin_points
            x = absolute_position_wheels(1, j, i);
            y = absolute_position_wheels(2, j, i);
            z = absolute_position_wheels(3, j, i);
            H = eval(h);
            nabla = eval(grad);
            ...
            if H <= 0
                ...
                velocity_tan = absolute_velocity_wheels(:, j, i) - direction * (absolute_velocity_wheels(:, j, i)' * direction);
    ...
    for i = 1:4
        for j = 1:num_rin_points
            distance1 = absolute_position_wheels(:, j, i) - absolute_position_body(:, 2*i-1);
            distance2 = absolute_position_wheels(:, j, i) - absolute_position_body(:, 2*i);
...
%     frame = plot_vehicle(absolute_position_wheels, absolute_position_body);
    ...
    if mod(clock, save_every) == 0
        ...
        body_velocities(:, clock / save_every) = velocity_cm_body;
    end
end

save("positions.mat", "wheels_positions", "body_positions")
