...
position_cm_wheels = [body_len, body_len, -body_len, -body_len;
       ...
       0, 0, 0, 0];  % initial position of centers of mass of wheels
position_cm_body = [0; 0; 1.154]; % initial position of center of mass of body
...
friction_coef = 0.9;  % coefficient of friction of the ground(m^-1)
angular_velocity_wheels = repmat([0; 0; 0], 1, 4); % initial angular velocity of wheels
...
rotation = [
    cos(slope_angle), 0,  sin(slope_angle);
    0, 1, 0;
    -sin(slope_angle), 0, cos(slope_angle)
    ];
position_tilde_body = rotation * position_tilde_body;
position_cm_wheels = rotation * position_cm_wheels;
position_cm_wheels = position_cm_wheels + position_cm_body;
% parfor i = 1:4
%     position_tilde_wheels(:, :, i) = rotation * position_tilde_wheels(:, :, i)
% end

% angular_velocity_recored
...
%             force_body(:, i*2) = force_body(:, i*2) - tension2 * ((distance2) / norm(distance2));
    ...
    angular_momentum_wheels = angular_momentum_wheels + squeeze(sum(cross(position_tilde_wheels, force_wheels, 1), 2) * dt);
    angular_momentum_wheels = angular_momentum_wheels + [0; 2; 0] * dt;
...
save("positions.mat", "wheels_positions", "body_positions", "body_velocities", "timestep", "save_every")
